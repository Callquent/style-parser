...
class CSSConverter {
  ...
  /**
   * Range les propriétés plates dans leurs groupes selon groups.yaml.
   * Retourne { type?, <group>?: {}, … }
   *
   * Les groupes sont lus dynamiquement depuis GroupLoader : ajouter/déplacer
   * une propriété dans groups.yaml suffit pour changer le bucketing.
   */
  _groupProps(props) {
    ...
    for (const [g, p] of Object.entries(buckets)) {
      ...
      // Le groupe "position" est sérialisé comme objet structuré { type, offset, z-index }
      if (g === 'position') {
        const posObj = {};
