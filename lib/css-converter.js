...
const { loader: groupLoader } = require('./group-loader');
...
class CSSConverter {
  ...
  _convert(cssText) {
    ...
    for (const [sel, comp] of Object.entries(components)) {
      ...
      const entry = { type: selectorType, attrs: attrsProps };
  ...
  /**
   * Sérialise le bloc selectors: en tête du fichier .ycss.
   * Lit selectors.yaml via la même résolution que _loadSelectors().
   */
  _serializeSelectorsBlock(lines) {
    ...
    for (const candidate of candidates) {
      if (fs.existsSync(candidate)) {
        try {
          ...
          const sels = doc.selectors ?? {};
          if (!Object.keys(sels).length) return;
          lines.push('selectors:');
          for (const [category, entries] of Object.entries(sels)) {
            lines.push(`  ${category}:`);
            for (const entry of (entries ?? [])) {
              const needsQuotes = /[:#*[\]{}]/.test(entry);
              lines.push(needsQuotes ? `    - "${entry}"` : `    - ${entry}`);
            }
          }
          lines.push('');
          return;
        } catch (_) { /* fichier corrompu, on ignore */ }
      }
    }
  }
  ...
  _toStyleFile({ variables, breakpoints, mediaVariables, components, extends: extendsSection, skipped }) {
    ...
    // ── Selectors ─────────────────────────────────────────────────────────────
    this._serializeSelectorsBlock(lines);
  ...
  _serializeComponentBlock(entry, lines, indent) {
    ...
    for (const [key, value] of Object.entries(entry)) {
      if (STRUCTURAL_KEYS.has(key) || key === 'attrs' || key === 'type') continue;
        ...
        try {
          ...
          for (const [category, entries] of Object.entries(doc.selectors ?? {})) {
            for (const entry of (entries ?? [])) {
              ...
              const name = String(entry).replace(/^::?/, '').replace(/\(.*\)$/, '').trim();
