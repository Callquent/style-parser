...
class CSSConverter {
  ...
  _toStyleFile({ variables, breakpoints, mediaVariables, components, extends: extendsSection, skipped, combinatorsMap }) {
    ...
    if (extendsSection != null) {
      ...
      for (const [name, entry] of Object.entries(extendsSection)) {
        lines.push('');
        // Quoter les noms qui commencent par @ (invalide en YAML sans guillemets)
    ...
    if (combinatorsMap.length) {
          ...
          for (const [targetName, { byPseudo, base, descendants }] of targetsMap) {
            ...
            for (const [category, pseudosMap] of byCategory) {
              ...
              for (const [pseudoName, grouped] of pseudosMap) {
                lines.push(`${' '.repeat(attrsIndent + 2)}${pseudoName}:`);
                if (Object.keys(grouped).length) {
                  lines.push(`${' '.repeat(attrsIndent + 4)}style:`);
                  this._serializeStyleInline(grouped, lines, attrsIndent + 6);
                }
              }
            }
      ...
      const _serializeDescendants = (descMap, lines, indent) => {
        ...
        for (const [name, { byPseudo, base, descendants: subDesc }] of descMap) {
          ...
          for (const [category, pseudosMap] of byCategory) {
            ...
            for (const [pseudoName, grouped] of pseudosMap) {
              lines.push(`${' '.repeat(childIndent + 2)}${pseudoName}:`);
              if (Object.keys(grouped).length) {
                lines.push(`${' '.repeat(childIndent + 4)}style:`);
                this._serializeStyleInline(grouped, lines, childIndent + 6);
              }
            }
          }
      ...
      // ── Injection de l'arbre des combinateurs dans les composants existants ──
      // Chaque nœud du tree est injecté à la fin du bloc existant (à n'importe
      // quel niveau d'indentation), ou créé comme nouvelle entrée racine.

      const _stripPrefix = s => (s.startsWith('.') || s.startsWith('#')) ? s.slice(1) : s;

      /**
       * Cherche la dernière occurrence d'une clé YAML dans lines[],
       * à n'importe quel niveau d'indentation.
       * Retourne { lineIdx, indent } ou null.
       */
      const _findKey = (name) => {
        // On accepte n'importe quel nombre d'espaces devant
        const plain   = `${name}:`;
        const quoted  = `"${name}":`;
        for (let i = lines.length - 1; i >= 0; i--) {
          const trimmed = lines[i].trimStart();
          if (trimmed === plain || trimmed === quoted) {
            const indent = lines[i].length - trimmed.length;
            return { lineIdx: i, indent };
          }
        }
        return null;
      };
      ...
      /**
       * Fin du bloc qui commence à startIdx :
       * prochaine ligne dont l'indentation est ≤ celle de startIdx (ou fin).
       */
      const _findBlockEnd = (startIdx, blockIndent) => {
        for (let i = startIdx + 1; i < lines.length; i++) {
          const l = lines[i];
          if (l.trim() === '') continue; // ignorer les lignes vides
          const ind = l.length - l.trimStart().length;
          if (ind <= blockIndent) return i;
        }
        return lines.length;
      };

      /**
       * Collecte les clés directement enfants d'un bloc (indent = blockIndent + 2).
       */
      const _childKeys = (startIdx, blockIndent) => {
        const childIndent = blockIndent + 2;
        const endIdx = _findBlockEnd(startIdx, blockIndent);
        const keys = new Set();
      ...
      /**
       * Sérialise récursivement un nœud de l'arbre en lignes YAML.
       * Filtre les enfants dont la clé est déjà présente dans existingKeys.
       */
      const _serializeTreeNode = (node, existingKeys, outLines, indent) => {
        const { children, combinators } = node;
        if (combinators.size) _serializeCombinatorEntries(combinators, outLines, indent);
        for (const [childSeg, childNode] of children) {
          const childNorm = _stripPrefix(childSeg);
          if (existingKeys.has(childNorm) || existingKeys.has(childSeg)) continue;
          const childKey = _segKey(childSeg);
          outLines.push(`${' '.repeat(indent)}${childKey}:`);
          _serializeTreeNode(childNode, new Set(), outLines, indent + 2);
        }
      };

      // Parcourir chaque chemin de l'arbre niveau par niveau.
      // Pour chaque nœud de l'arbre on cherche la clé dans lines[].
      // On injecte au niveau le plus profond trouvé.
      const _injectNode = (seg, node, fallbackIndent) => {
        const norm   = _stripPrefix(seg);
        const found  = _findKey(norm) ?? _findKey(seg);

        if (found) {
          const { lineIdx, indent } = found;
          const endIdx  = _findBlockEnd(lineIdx, indent);
          const existing = _childKeys(lineIdx, indent);
          const extraLines = [];
          _serializeTreeNode(node, existing, extraLines, indent + 2);
          if (extraLines.length) lines.splice(endIdx, 0, ...extraLines);
        } else {
          // Clé introuvable → créer au niveau fallbackIndent
          const pad = ' '.repeat(fallbackIndent);
          const key = _segKey(seg);
          lines.push('');
          lines.push(`${pad}${key}:`);
          _serializeTreeNode(node, new Set(), lines, fallbackIndent + 2);
        }
      };

      for (const [seg, node] of tree) {
        _injectNode(seg, node, 2); // niveau racine = 2 espaces
      }
    }
  ...
  _serializeComponentBlock(entry, lines, indent) {
    ...
    if (entry.extends != null) {
      ...
      const safeRef = ref.startsWith('@') ? `"${ref}"` : ref;
      lines.push(`${pad}extends: ${safeRef}`);
    }
    ...
    // 2. {category}: (bloc YAML normal, sans wrapper selectors:)
