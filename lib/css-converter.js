...
const GROUPS = {
  ...
  typo:         new Set(['size','weight','font-family','font-style','line-height','text-transform','text-align','text-decoration','white-space','text-overflow']),
  uiProps:      new Set(['background','background-image','color']),
  position:     new Set(['position','top','right','bottom','left','z-index']),
...
class CSSConverter {
  ...
  _groupProps(props) {
    const top     = {};
    const buckets = { layout: {}, box: {}, typo: {}, uiProps: {}, position: {}, interactions: {} };
    const rest    = {};
    ...
    for (const [g, p] of Object.entries(buckets)) {
      ...
      // Position group: restructure as { type, offset: { top, right, bottom, left } }
      if (g === 'position') {
        const posObj = {};
        if (p['position'] !== undefined) posObj['type'] = p['position'];
        const offsets = {};
        for (const side of ['top', 'right', 'bottom', 'left']) {
          if (p[side] !== undefined) offsets[side] = p[side];
        }
        if (Object.keys(offsets).length) posObj['offset'] = offsets;
        if (p['z-index'] !== undefined) posObj['z-index'] = p['z-index'];
        grouped[g] = posObj;
      } else {
        grouped[g] = p;
      }
  ...
  _parseSelector(rawSel) {
    ...
    // Reject selectors with sibling combinators (+, ~) or universal selector (*)
    // These can't be meaningfully mapped to a component/child model
    if (/\s*[+~]\s*/.test(sel)) return { type: 'complex', raw: rawSel };
    if (/(?:^|\s)\*(?:\s|$|\[|:)/.test(sel)) return { type: 'complex', raw: rawSel };

    // Normalize child combinator > to a space
    ...
    if (!state) {
      ...
      if (pcm) {
        ...
        const KNOWN_PSEUDO = new Set([
          'hover','focus','active','checked','disabled','placeholder',
          'focus-within','focus-visible','before','after',
  ...
  _convert(cssText) {
      ...
      if (Object.keys(comp.media).length) {
        ...
        for (const [q, mb] of Object.entries(comp.media)) {
          ...
          for (const childName of mqChildNames) {
            ...
            const childMqEntry = { attrs: this._groupProps({ ...ci, ...cl }) };
            const cs = mqChildStates[childName] ?? {};
            if (Object.keys(cs).length) {
              childMqEntry.states = {};
              for (const [s, d] of Object.entries(cs)) {
                const { intents: si, leftovers: sl } = this._reverseMap({ ...d });
                childMqEntry.states[s] = { attrs: this._groupProps({ ...si, ...sl }) };
              }
            }
    ...
    return { variables: rt.base, breakpoints: rt.breakpoints, mediaVariables: rt.media, extends: sharedComps, components: deduped, skipped };
  }
  ...
  _toStyleFile({ variables, breakpoints, mediaVariables, components, extends: extendsSection, skipped }) {
    const lines = [];
