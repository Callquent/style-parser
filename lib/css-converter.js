...
class CSSConverter {
  ...
  _toStyleFile({ variables, breakpoints, mediaVariables, components, extends: extendsSection, skipped, combinatorsMap }) {
    ...
    if (combinatorsMap.length) {
      ...
      const _decompose = (sel, combinator) => {
        ...
        // Nom de clé YAML pour la cible : strip . ou #, garder le reste tel quel
        let targetName = targetSeg;
        if (targetName.startsWith('.')) targetName = targetName.slice(1);
        else if (targetName.startsWith('#')) targetName = targetName.slice(1);

        return { parts, combinator, targetName, pseudo, pseudoCategory };
      };
       ...
       *   }>
       */
      const _insert = (tree, parts, combinator, targetName, pseudo, pseudoCategory, grouped) => {
        ...
        if (!targetsMap.has(targetName)) targetsMap.set(targetName, { byPseudo: new Map(), base: null });
      ...
      for (const { sel, combinator, decls } of combinatorsMap) {
        ...
        _insert(tree, dec.parts, dec.combinator, dec.targetName, dec.pseudo, dec.pseudoCategory, grouped);
      }
      ...
      const _serializeCombinatorEntries = (combinatorsMap, lines, indent) => {
        ...
        for (const [combinatorChar, targetsMap] of combinatorsMap) {
          ...
          for (const [targetName, { byPseudo, base }] of targetsMap) {
            ...
            if (base && Object.keys(base).length) {
              lines.push(`${' '.repeat(attrsIndent)}attrs:`);
              this._serializeAttrsInline(base, lines, attrsIndent + 2);
            }
            ...
            for (const [category, pseudosMap] of byCategory) {
              ...
              for (const [pseudoName, grouped] of pseudosMap) {
                ...
                if (Object.keys(grouped).length) {
                  lines.push(`${' '.repeat(attrsIndent + 6)}attrs:`);
                  this._serializeAttrsInline(grouped, lines, attrsIndent + 8);
                }
              }
            }
          }
   ...
   *   "              padding: 8px"
   */
  _serializeAttrsInline(grouped, lines, indent) {
    const pad  = ' '.repeat(indent);
