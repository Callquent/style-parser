...
const MAPPERS = {
  ...
  // position — accepte soit un scalaire (rétrocompat) soit { type, offset, z-index }
  position(value) {
    if (typeof value === 'string') return { position: value };
    const css = {};
    if (value.type)              css['position'] = value.type;
    if (value.offset) {
      for (const side of ['top', 'right', 'bottom', 'left']) {
        if (value.offset[side] !== undefined) css[side] = value.offset[side];
      }
    }
    if (value['z-index'] !== undefined) css['z-index'] = value['z-index'];
    return css;
  },
  // Scalaires autonomes (hors groupe position, rétrocompat)
...
function mapProperty(key, value) {
  ...
  // position accepts an object { type, offset, z-index }
  if (typeof value === 'object' && !Array.isArray(value) && key !== 'position') return {};
