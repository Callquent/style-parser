...
class Linter {
  ...
  _lintVariables(variables) {
    for (const [name, value] of Object.entries(variables)) {
      ...
      // Avertir les variables qui semblent être des couleurs sans guillemets hex corrects
  ...
  _lintComponents(components, variables) {
    ...
    for (const [name, def] of Object.entries(components)) {
      ...
      this._lintProps(def, path, variables, names);

      // States
      if (def.states) {
        this._lintStates(def.states, path, variables);
      }
    }
  }
  ...
  _lintProps(def, basePath, variables, componentNames) {
    ...
    for (const [key, value] of Object.entries(def)) {
      ...
      if (typeof value === 'object' && !Array.isArray(value) && value !== null) {
        if (key !== 'states') {
          this._lintProps(value, path, variables, componentNames);
        }
        continue;
      }
        ...
        if (/^[a-z][a-z0-9-]+$/.test(value) && !value.includes(' ') && value.length > 2) {
          ...
          if (!CSS_KEYWORDS.has(value)) {
            this._info('possible-missing-token',
              `"${value}" ressemble à un token mais n'est pas déclaré dans variables.`,
              path);
          }
        }
      }
  ...
  _lintStates(states, basePath, variables) {
    ...
    for (const [state, props] of Object.entries(states)) {
      ...
      if (props && typeof props === 'object') {
        this._lintProps(props, path, variables, []);
      }
    }
  }
