...
class StyleParser {
  ...
  _parseName(rawKey, def = {}) {
    if (rawKey.startsWith('@')) {
      ...
      return { name: rawKey.slice(1), selectorType: attrsType === 'id' ? 'id' : 'class', shared: true };
    }
    ...
    const attrsType = def.attrs?.type ?? def.type;
  ...
  _applyExtends(def) {
    ...
    return { ...def, attrs: this._deepMerge(resolvedParent.attrs ?? {}, def.attrs ?? {}) };
  }
  ...
  _extractCSS(def) {
    const flat  = this._flattenGroups(def.attrs ?? {});
    const plain = {};
  ...
  _extractStates(def) {
    ...
    for (const [state, stateBlock] of Object.entries(def.states)) {
      ...
      result[state] = this._mapWithPlugins(this.resolver.resolveProps(this._flattenGroups(attrs)));
    }
    return result;
  }
  ...
  _extractMedia(def) {
    ...
    for (const [query, block] of Object.entries(def.media)) {
      ...
      const legacyProps = Object.fromEntries(Object.entries(block).filter(([k]) => k !== 'attrs' && k !== 'states'));
      const css         = this._mapWithPlugins(this.resolver.resolveProps(this._flattenGroups({ ...legacyProps, ...attrsBlock })));
      const states      = {};
      if (statesBlock) {
        for (const [state, stateBlock] of Object.entries(statesBlock)) {
          const attrs = stateBlock?.attrs ?? stateBlock ?? {};
          states[state] = this._mapWithPlugins(this.resolver.resolveProps(this._flattenGroups(attrs)));
        }
      }
    ...
    for (const [key, value] of Object.entries(def)) {
      ...
      if (childStates && typeof childStates === 'object') {
        for (const [state, stateBlock] of Object.entries(childStates)) {
          const attrs = stateBlock?.attrs ?? stateBlock ?? {};
          statesCss[state] = this._mapWithPlugins(this.resolver.resolveProps(this._flattenGroups(attrs)));
        }
      }
      const media = {};
      if (childMedia && typeof childMedia === 'object') {
        for (const [query, block] of Object.entries(childMedia)) {
          ...
          const attrsBlock  = block.attrs ?? {};
          const legacyProps = Object.fromEntries(Object.entries(block).filter(([k]) => k !== 'attrs' && k !== 'states'));
          media[query] = { css: this._mapWithPlugins(this.resolver.resolveProps(this._flattenGroups({ ...legacyProps, ...attrsBlock }))) };
        }
      }
