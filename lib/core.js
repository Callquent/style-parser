...
// ══════════════════════════════════════════════════════════════════════════════
// GroupLoader
// Charge style.yaml et expose les groupes de propriétés (layout, box, typo…).
...
class GroupLoader {
  ...
  _resolve(explicit) {
    ...
    // Remonte depuis __dirname jusqu'à trouver style.yaml hors node_modules
    let dir = __dirname;
    while (true) {
      const candidate = path.join(dir, 'style.yaml');
    ...
    const fromCwd = path.join(process.cwd(), 'style.yaml');
    if (fs.existsSync(fromCwd)) return fromCwd;

    const fromDir = path.join(__dirname, 'style.yaml');
