 ...
 *   layout, box, typo, appearance, interactions
 *
...
const PROP_GROUPS   = new Set(['layout', 'box', 'typo', 'appearance', 'interactions']);
...
class StyleParser {
  ...
  parseFile(filePath) {
    ...
    return this._build(tokens, components, source, rawYaml.extends ?? {});
  }
  ...
  _build(tokens, components, source, extendsSection = {}) {
    this.tokens        = tokens;
  ...
  _resolveAll() {
    ...
    for (const [rawKey, def] of Object.entries(this.rawComponents)) {
      // Les composants partagés (@xxx) sont résolus mais non émis en CSS ;
      // on les garde dans la map pour que les enfants puissent les référencer.
