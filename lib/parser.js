...
class StyleParser {
  ...
  /**
   * Déplie les groupes de propriétés définis dans groups.yaml
   * présents dans un objet d'attributs.
   *
   *   { box: { padding: '16px' }, uiProps: { background: '#fff' } }
   *   → { padding: '16px', background: '#fff' }
   */
  _flattenGroups(attrs) {
    const { groupNames } = groupLoader.load();
    const propGroupNames = new Set(groupNames);
    const flat = {};
    for (const [key, value] of Object.entries(attrs)) {
      if (propGroupNames.has(key) && value && typeof value === 'object' && !Array.isArray(value)) {
